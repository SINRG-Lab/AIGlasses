...
async def tts_send_and_receive(socket_id, http_request, timeout=500):
...
# ============== GEMINI TTS API ==============
async def gemini_tts(text, voice="Kore"):
    """
    ...
    start_time = time.ticks_ms()
    audio_b64 = await tts_send_and_receive(socket_id, http_request, timeout=120)
    latency = time.ticks_diff(time.ticks_ms(), start_time)
