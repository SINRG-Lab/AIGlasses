"""
Walter ESP32-S3 MicroPython - Gemini Audio-to-Text
Sends audio directly to Gemini API (no Deepgram) for transcription/response
...
async def send_and_receive(socket_id, http_request, timeout=120):
    """Socket send/receive optimized for large audio payloads"""
    
    request_bytes = http_request if isinstance(http_request, bytes) else http_request.encode('utf-8')
    
    # Larger chunks and minimal delay for audio upload
    chunk_size = 2048
    delay = 0.01
    
    total_size = len(request_bytes)
    print(f"Sending {total_size} bytes...")
...
async def setup():
    print("\n" + "="*50)
    print("Walter - Gemini Audio-to-Text Latency Benchmark")
    print("="*50)
...
# ESP32 Gemini Audio-to-Text Latencies (LTE) - in milliseconds
latencies = [""")
...
ax.plot(sorted_data, cdf, 'm-', linewidth=2, label='ESP32 (LTE) - Gemini Audio-to-Text')
ax.scatter(sorted_data, cdf, c='purple', s=30, alpha=0.6)
ax.axhline(y=0.5, color='gray', linestyle=':', alpha=0.5)
ax.axhline(y=0.95, color='gray', linestyle=':', alpha=0.5)
ax.set_xlabel('Latency (ms)', fontsize=12)
ax.set_ylabel('CDF', fontsize=12)
ax.set_title('ESP32 Gemini Audio-to-Text Latency CDF (over LTE)', fontsize=14)
...
plt.tight_layout()
plt.savefig('cdf_esp32_gemini_audio.png', dpi=150)
plt.show()
""")
        print("="*50)
